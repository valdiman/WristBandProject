...
wt <- data.frame(read_excel("Data/SamplesWBT.xlsx", sheet = "samples",
                            col_names = TRUE, col_types = NULL))
...
# Save map in folder
ggsave("Output/Plots/FreqPCBWT.png", plot = plot.cong.freq,
...
wt_common <- wt.2[, intersect(colnames(wt.2), congener_names)]
# Subset sr to include only the common congeners
sr_common <- sr[sr$congener %in% colnames(wt_common), ]
# Ensure that sr_common is ordered to match the columns of wb_common
sr_common <- sr_common[match(colnames(wt_common), sr_common$congener), ]
# Divide each element in wb_common by corresponding element in sr_common$Average_Sampling_Rate
wt_div_sr <- sweep(wt_common, 2, sr_common$Average_Sampling_Rate, FUN = "/")
# !!! Use a constant sampling rate
# wb_div_sr <- sweep(wb_common, 2, 1.5, FUN = "/")
# Extract time.day from wb.wr
wt_time_day <- wt$time.day
# Divide wb_div_sr further by corresponding value in wb_time_day
conc.wt <- sweep(wt_div_sr, 1, wt_time_day, FUN = "/")
# Add school time
conc.wt <- cbind(wt$school.year, conc.wt)
# Add teacher code
conc.wt <- cbind(wt$code.teacher, conc.wt)
# Change name
names(conc.wt)[names(conc.wt) == 'wt$school.year'] <- 'school.year'
names(conc.wt)[names(conc.wt) == 'wt$code.teacher'] <- 'code.teacher'

# Predicted Total PCB Concentration ---------------------------------------
tPCB.conc.wt <- as.data.frame(rowSums(conc.wt[, 3:151], na.rm = TRUE))
tPCB.conc.wt$code.teacher <- conc.wt$code.teacher
tPCB.conc.wt$school.year <- conc.wt$school.year
# Change columns names
colnames(tPCB.conc.wt) <- c("tPCB", "code.teacher", "school.year")

# See
tPCB.conc.wt
min(tPCB.conc.wt$tPCB)
max(tPCB.conc.wt$tPCB)

# Need to change format to include duplicate
tPCB.conc.wt <- tPCB.conc.wt %>%
  arrange(code.teacher) %>%
  mutate(ID = row_number())

ggplot(tPCB.conc.wt, aes(x = factor(ID), y = tPCB, fill = code.teacher)) +
  geom_bar(stat = 'identity', width = 0.8, fill = "black") +
  scale_x_discrete(labels = tPCB.conc.wt$code.teacher) +
  theme_bw() +
  theme(aspect.ratio = 6/35) +
...
# Save plot in folder
ggsave("Output/Plots/WTtPCB.png", plot = tPCB.wt,
...
# Save plot in folder
ggsave("Output/Plots/WTtPCByr.png", plot = tPCB.wt.yr,
       width = 10, height = 5, dpi = 1200)
...
# Calculate means and standard deviations for columns 3 to 151
median_conc <- apply(conc.wt[, 3:151], 2, median, na.rm = TRUE)
mean_conc <- colMeans(conc.wt[, 3:151], na.rm = TRUE)
st_conc <- apply(conc.wt[, 3:151], 2, sd, na.rm = TRUE)
cov_conc <- st_conc / mean_conc * 100

PCB.conc.wt.ave <- data.frame(
...
min(PCB.conc.wt.ave$Mean_Conc)
max(PCB.conc.wt.ave$Mean_Conc)
mean(PCB.conc.wt.ave$COV)

# Top 10
top_10_PCB.means <- PCB.conc.wt.ave %>%
  arrange(desc(Mean_Conc)) %>%
  head(10)
print(top_10_PCB.means)

# Plot PCBi
plot.pcb.data <- conc.wt[, 3:151]
...
# Save plot in folder
ggsave("Output/Plots/WTPCBi.png", plot = plot.pcbi,
       width = 10, height = 5, dpi = 1200)

# Concentration Profile Analysis ------------------------------------------
# Profiles
tmp <- rowSums(conc.wt[, 3:151], na.rm = TRUE)
prof.wt.conc <- sweep(conc.wt[, 3:151], 1, tmp, FUN = "/")
prof.wt.conc <- cbind(conc.wt$code.teacher, prof.wt.conc)
# Check sum of all PCBs (i.e., = 1)
rowSums(prof.wt.conc[, 2:150], na.rm = TRUE)

# See top PCBi for each sample
# Extract numeric columns and their names
numeric_data <- prof.wt.conc[, -1]
...
top_10_PCBi_df <- cbind(prof.wt.conc[, 1], top_10_PCBi_df)

# Rename columns
colnames(top_10_PCBi_df) <- c("code.teacher", paste0("Top10_PCBi_", 1:10))

# Add school built year
top_10_PCBi_df <- cbind(conc.wt$school.year, top_10_PCBi_df)

# Create the plot
ggplot(top_10_PCBi_df, aes(x = Top10_PCBi_1, y = `conc.wt$school.year`)) +
...
# PCA
t.prof.wt.conc <- data.frame(t(prof.wt.conc[, 2:150]))
colnames(t.prof.wt.conc) <- conc.wt$code.teacher
PCA <- prcomp(t.prof.wt.conc, scale. = TRUE)
summary(PCA)

# Plot PCA results
ggplot2::autoplot(PCA, data = t.prof.wt.conc, size = 1.5, shape = 21) +
  theme_bw() +
  xlim(-0.4, 0.6) +
  ylim(-0.4, 0.6) +
...
# Create matrix to storage results
costheta.samples <- matrix(nrow = length(t.prof.wt.conc[1, ]),
                           ncol = length(t.prof.wt.conc[1, ]))

for (i in 1:length(t.prof.wt.conc[1,])) {
  for (j in 1:length(t.prof.wt.conc[1,])) {
    m1 <- t.prof.wt.conc[, i]
    m2 <- t.prof.wt.conc[, j]
...
colnames(costheta.samples) <- colnames(t.prof.wt.conc)
# Add row names with samples name
rownames(costheta.samples) <- colnames(t.prof.wt.conc)
costheta_df <- as.data.frame(costheta.samples)
# Add conc.wt$school.year as a new column to the data frame
costheta_df$school.year <- conc.wt$school.year

# Export
write.csv(costheta_df, file = "Output/Data/csv/CosineThetaTeachers.csv")

# Cosine theta visualization
# Calculate tPCB values to be added to the plot
tPCB.conc.wt <- as.data.frame(rowSums(conc.wt[, 3:151], na.rm = TRUE))
colnames(tPCB.conc.wt) <- "tPCB"

# Add the tPCB values to costheta_df
costheta_df <- bind_cols(costheta_df, tPCB.conc.wt)
...
costheta_long <- costheta_df %>%
  ...
  mutate(Var2 = rep(colnames(costheta_df)[-c(37, 38)], each = 36))  # Exclude 'school.year' and 'tPCB'

# Add the correct year and tPCB values to Var1 and Var2
costheta_long <- costheta_long %>%
  mutate(Var1_year = years[match(Var1, colnames(costheta_df)[-c(37, 38)])],
         Var2_year = years[match(Var2, colnames(costheta_df)[-c(37, 38)])],
         Var1_tPCB = round(tPCB[match(Var1, colnames(costheta_df)[-c(37, 38)])], 2),
         Var2_tPCB = round(tPCB[match(Var2, colnames(costheta_df)[-c(37, 38)])], 2)) %>%
  mutate(Var1 = paste0(Var1, " (", Var1_year, ", ", Var1_tPCB, ")"),
         Var2 = paste0(Var2, " (", Var2_year, ", ", Var2_tPCB, ")")) %>%
  select(-Var1_year, -Var2_year, -Var1_tPCB, -Var2_tPCB)

# Remove NAs from the long data
costheta_long <- na.omit(costheta_long)
...
# Plot the data
plot.cos.theta.low <- ggplot(data = costheta_correlations[1:16, ], aes(x = Var1, y = Var2, fill = Value)) +
...
# Save plot
ggsave("Output/Plots/Profiles/CosThetaLowV01.png", plot = plot.cos.theta.low,
       width = 10, height = 10, dpi = 1200)

plot.cos.theta.high <- ggplot(data = costheta_correlations[613:630, ], aes(x = Var1, y = Var2, fill = Value)) +
  geom_tile(color = "white") +
  scale_fill_gradient(low = "blue", high = "red") +
  labs(x = "", y = "") +
  theme_minimal() +
...
# Save plot
ggsave("Output/Plots/Profiles/CosThetaHighV01.png", plot = plot.cos.theta.high,
       width = 10, height = 10, dpi = 1200)

# Plot Individual PCB Profiles --------------------------------------------
# Select rows for the sample
selected_rows <- prof.wt.conc %>%
  filter(conc.wt$code.teacher %in% c("wt.06.l")) # need to change the sample
...
# Save plot
ggsave("Output/Plots/Profiles/wt25r.png", plot = plot.25.r,
