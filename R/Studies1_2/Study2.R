## Script to calculate individual PCB apparent sampling rates
...
# Export results
write.csv(SR.V1.nd,
          file = "Output/Data/Studies1_2/Study2/SR.V1.nd.csv",
          row.names = FALSE)
...
# Export results
write.csv(SR.V1.d,
          file = "Output/Data/Studies1_2/Study2/SR.V1.d.csv",
          row.names = FALSE)
...
p.sr.V1.koa.1 <- ggplot(sr.ave.V1, aes(x = logKoa, y = ave_sr)) +
  ...
  annotate("text", x = 7, y = 15.7,
           label = paste("Ave. V1 (d & nd)"),size = 4) +
  ...
  ylab(expression(bold("Ave Apparent Sampling Rate (m"^3*"/d)"))) +
  theme(axis.text.y = element_text(face = "bold", size = 10),
        axis.title.y = element_text(face = "bold", size = 10)) +
...
# Save plot in folder
ggsave("Output/Plots/Studies1_2/Study2/V1_logKoa1.png", plot = p.sr.V1.koa.1,
...
p.sr.V1.koa.2 <- ggplot(sr.long.V1, aes(x = logKoa, y = sr)) +
  geom_point(size = 3, shape = 1, stroke = 1) +
  geom_smooth(method = "lm", formula = y ~ exp(x), se = FALSE, color = "blue") +
  annotate("text", x = min(sr.long.V1$logKoa) + 0.45, y = max(sr.long.V1$sr) * 1.2,
           label = paste("V1 (d & nd)"),size = 5) +
  ...
  theme(aspect.ratio = 1) +
  xlab(expression(bold("log "*italic(K)[OA]))) +
  ylab(expression(bold("Apparent Sampling Rate (m"^3*"/d)"))) +
...
# Save plot in folder
ggsave("Output/Plots/Studies1_2/Study2/V1_logKoa.png", plot = p.sr.V1.koa.2,
       width = 6, height = 6, dpi = 500)
...
# Export results
write.csv(SR.V2.d,
          file = "Output/Data/Studies1_2/Study2/SR.V2.d.csv", row.names = FALSE)
...
p.sr.V2.koa <- ggplot(sr.V2, aes(x = logKoa, y = sr)) +
  geom_point(size = 3, shape = 1, stroke = 1) +
  geom_smooth(method = "lm", formula = y ~ exp(x), se = FALSE, color = "blue") +
  annotate("text", x = 6.45, y = 7, label = paste("V2 (d)"),size = 5) +
...
# Save plot in folder
ggsave("Output/Plots/Studies1_2/Study2/V2_logKoa.png", plot = p.sr.V2.koa,
       width = 6, height = 6, dpi = 500)
...
write.csv(SR.V3.1st.nd,
          file = "Output/Data/Studies1_2/Study2/SR.V3.1st.nd.csv",
          row.names = FALSE)
...
write.csv(SR.V3.2nd.nd,
          file = "Output/Data/Studies1_2/Study2/SR.V3.2nd.nd.csv",
          row.names = FALSE)
...
p.sr.V3.koa.1 <- ggplot(sr.ave.V3, aes(x = logKoa, y = ave_sr)) +
  ...
  annotate("text", x = 7.6, y = 7,
           label = paste("Ave. V3 (nd 1st & 2nd weeks)"),size = 5) +
  ...
  theme(aspect.ratio = 1) +
  xlab(expression(bold("log "*italic(K)[OA]))) +
  ylab(expression(bold("Average Apparent Sampling Rate (m"^3*"/d)"))) +
  theme(axis.text.y = element_text(face = "bold", size = 12),
        axis.title.y = element_text(face = "bold", size = 12)) +
  theme(axis.text.x = element_text(face = "bold", size = 12),
        axis.title.x = element_text(face = "bold", size = 12))
...
# Save plot in folder
ggsave("Output/Plots/Studies1_2/Study2/V3_logKoa.nd.1.png",
...
p.sr.V3.koa.2 <- ggplot(sr.long.V3, aes(x = logKoa, y = sr)) +
  ...
  annotate("text", x = 7.3, y = 7,
           label = paste("V3 (nd 1st & 2nd weeks)"),size = 5) +
  annotate("text", x = min(sr.long.V3$logKoa) + 1.5, y = max(sr.long.V3$sr) * 1.2,
           label = paste("sr =", round(a, 3), "* exp(", round(b, 2), "* log Koa)"),
           size = 5) +
...
# Save plot in folder
ggsave("Output/Plots/Studies1_2/Study2/V3_logKoa.nd.2.png",
...
p.sr.V3.koa.3 <- ggplot(sr.long.V3.1st, aes(x = logKoa, y = sr)) +
  ...
  xlim(7, 11) +
  annotate("text", x = 7.6, y = 7,
           label = paste("V3 (nd 1st week)"),size = 5) +
...
# Save plot in folder
ggsave("Output/Plots/Studies1_2/Study2/V3_logKoa.nd.1st.week.png",
...
p.sr.V3.koa.4 <- ggplot(sr.long.V3.2nd, aes(x = logKoa, y = sr)) +
  ...
  annotate("text", x = 7.4, y = 4.5,
           label = paste("V3 (nd 2nd week)"),size = 5) +
...
# Save plot in folder
ggsave("Output/Plots/Studies1_2/Study2/V3_logKoa.nd.2nd.week.png",
...
write.csv(SR_V3.2_nw, "Output/Data/Studies1_2/Study2/SR.V3.2_nw.csv", row.names = FALSE)
write.csv(SR_V3.2_w,  "Output/Data/Studies1_2/Study2/SR.V3.2_w.csv",  row.names = FALSE)
...
p.sr.V3.koa.nw <- ggplot(sr.V3.nw, aes(x = logKoa, y = sr)) +
  geom_point(size = 3, shape = 1, stroke = 1) +
  geom_smooth(method = "lm", formula = y ~ exp(x), se = FALSE, color = "blue") +
  annotate("text", x = 6.8, y = 5.4, label = paste("V3 (non-wiped)"),
           size = 5) +
  annotate("text", x = 7.7, y = 5.1,
           label = paste("sr = ", round(a, 3),
                         " * exp(", round(b, 2), " x log Koa)", sep = ""),
           size = 5) + 
  annotate("text", x = 6.55, y = 4.8,
           label = paste("R² = ", round(r2, 2)), size = 5) + 
  theme_bw() +
  theme(aspect.ratio = 1) +
  xlab(expression(bold("log "*italic(K)[OA]))) +
...
# Save plot in folder
ggsave("Output/Plots/Studies1_2/Study2/V3_logKoa.nw.png",
...
p.sr.V3.koa.w <- ggplot(sr.V3.w, aes(x = logKoa, y = sr)) +
  geom_point(size = 3, shape = 1, stroke = 1) +
  geom_smooth(method = "lm", formula = y ~ exp(x), se = FALSE, color = "blue") +
  annotate("text", x = 6.9, y = 6.2, label = paste("V3 (wiped)"),
           size = 5) +
...
# Save plot in folder
ggsave("Output/Plots/Studies1_2/Study2/V3_logKoa.w.png",
...
plot.18.30 <- plot.18.30 + 
  geom_point(aes(x = 1, y = 2.0), size = 6, shape = 21,
             fill = group_colors["V1 d"], color = "black",
             show.legend = FALSE) +
  annotate("text", x = 2, y = 2.0, label = paste0(
      "'V1 d' == ",
      round(slopes$slope[match("V1 d", slopes$group)], 2),
      " ~ '(m'^3*'/d)'"), parse = TRUE, hjust = 0, size = 10,
      color = "black") +
  annotate("point", x = 1, y = 1.8, shape = 21, size = 6,
           fill = group_colors["V1 nd"], color = "black") +
  annotate("text", x = 2, y = 1.8, label = paste0(
      "'V1 nd' == ",
      round(slopes$slope[match("V1 nd", slopes$group)], 2),
      " ~ '(m'^3*'/d)'"), parse = TRUE, hjust = 0, size = 10,
      color = "black") +
  geom_point(aes(x = 1, y = 1.6), size = 6, shape = 21,
             fill = group_colors["V2 d"], color = "black",
             show.legend = FALSE) +
  annotate("text", x = 2, y = 1.6, label = paste0(
      "'V2 d' == ",
      round(slopes$slope[match("V2 d", slopes$group)], 2),
      " ~ '(m'^3*'/d)'"), parse = TRUE, hjust = 0, size = 10,
      color = "black") +
...
# Save plot
ggsave("Output/Plots/Studies1_2/Study2/PCB18.30VoluntSamplingRates.png",
...
plot.52 <- plot.52 + 
  geom_point(aes(x = 1, y = 3.1), size = 6, shape = 21,
             fill = group_colors["V1 d"], color = "black",
             show.legend = FALSE) +
  annotate("text", x = 2, y = 3.1, label = paste0(
    "'V1 d' == ",
    round(slopes$slope[match("V1 d", slopes$group)], 2),
    " ~ '(m'^3*'/d)'"), parse = TRUE, hjust = 0, size = 10,
    color = "black") +
  geom_point(aes(x = 1, y = 2.9), size = 6, shape = 21,
             fill = group_colors["V1 nd"], color = "black",
             show.legend = FALSE) +
  annotate("text", x = 2, y = 2.9, label = paste0(
    "'V1 nd' == ",
    round(slopes$slope[match("V1 nd", slopes$group)], 2),
    " ~ '(m'^3*'/d)'"), parse = TRUE, hjust = 0, size = 10,
    color = "black") +
  geom_point(aes(x = 1, y = 2.7), size = 6, shape = 21,
             fill = group_colors["V2 d"], color = "black",
             show.legend = FALSE) +
  annotate("text", x = 2, y = 2.7, label = paste0(
    "'V2 d' == ",
    round(slopes$slope[match("V2 d", slopes$group)], 2),
    " ~ '(m'^3*'/d)'"), parse = TRUE, hjust = 0, size = 10,
    color = "black") +
...
# Save plot
ggsave("Output/Plots/Studies1_2/Study2/PCB52VoluntSamplingRates.png",
...
plot.118 <- plot.118 + 
  geom_point(aes(x = 1, y = 10.0), size = 6, shape = 21,
             fill = group_colors["V1 d"], color = "black",
             show.legend = FALSE) +
  annotate("text", x = 2, y = 10.0, label = paste0(
    "'V1 d' == ",
    round(slopes$slope[match("V1 d", slopes$group)], 2),
    " ~ '(m'^3*'/d)'"), parse = TRUE, hjust = 0, size = 10,
    color = "black") +
  geom_point(aes(x = 1, y = 9.25), size = 6, shape = 21,
             fill = group_colors["V1 nd"], color = "black",
             show.legend = FALSE) +
  annotate("text", x = 2, y = 9.25, label = paste0(
    "'V1 nd' == ",
    round(slopes$slope[match("V1 nd", slopes$group)], 2),
    " ~ '(m'^3*'/d)'"), parse = TRUE, hjust = 0, size = 10,
    color = "black") +
  geom_point(aes(x = 1, y = 8.5), size = 6, shape = 21,
             fill = group_colors["V2 d"], color = "black",
             show.legend = FALSE) +
  annotate("text", x = 2, y = 8.5, label = paste0(
    "'V2 d' == ",
    round(slopes$slope[match("V2 d", slopes$group)], 2),
    " ~ '(m'^3*'/d)'"), parse = TRUE, hjust = 0, size = 10,
    color = "black") +
...
# Save plot
ggsave("Output/Plots/Studies1_2/Study2/PCB118VoluntSamplingRates.png",
...
plot.187 <- plot.187 + 
  geom_point(aes(x = 1, y = 16.0), size = 6, shape = 21,
             fill = group_colors["V1 d"], color = "black",
             show.legend = FALSE) +
  annotate("text", x = 2, y = 16.0, label = paste0(
    "'V1 d' == ",
    round(slopes$slope[match("V1 d", slopes$group)], 2),
    " ~ '(m'^3*'/d)'"), parse = TRUE, hjust = 0, size = 10,
    color = "black") +
  geom_point(aes(x = 1, y = 15.0), size = 6, shape = 21,
             fill = group_colors["V1 nd"], color = "black",
             show.legend = FALSE) +
  annotate("text", x = 2, y = 15, label = paste0(
    "'V1 nd' == ",
    round(slopes$slope[match("V1 nd", slopes$group)], 2),
    " ~ '(m'^3*'/d)'"), parse = TRUE, hjust = 0, size = 10,
    color = "black") +
  geom_point(aes(x = 1, y = 14.0), size = 6, shape = 21,
             fill = group_colors["V2 d"], color = "black",
             show.legend = FALSE) +
  annotate("text", x = 2, y = 14, label = paste0(
    "'V2 d' == ",
    round(slopes$slope[match("V2 d", slopes$group)], 2),
    " ~ '(m'^3*'/d)'"), parse = TRUE, hjust = 0, size = 10,
    color = "black") +
...
# Save plot
ggsave("Output/Plots/Studies1_2/Study2/PCB187VoluntSamplingRates.png",
...
write.csv(SR_averages_sd_cv,
          file = "Output/Data/Studies1_2/Study2/PersonalAveSR.csv",
          row.names = FALSE)
...
# Save plot
ggsave("Output/Plots/Studies1_2/Study2/AvePersonalSRs.png",
...
# Save plot
ggsave("Output/Plots/Studies1_2/Study2/SRs.png",
