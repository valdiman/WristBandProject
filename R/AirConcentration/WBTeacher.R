...
tPCB.conc.wt <- as.data.frame(rowSums(conc.wt[, 3:136], na.rm = TRUE))
...
# Save plot in folder
ggsave("Output/Plots/Teachers/WTtPCB.png", plot = tPCB.wt,
...
# Save plot in folder
ggsave("Output/Plots/Teachers/WTtPCByr.png", plot = tPCB.wt.yr,
       width = 10, height = 5, dpi = 1200)
...
median_conc <- apply(conc.wt[, 3:136], 2, median, na.rm = TRUE)
mean_conc <- colMeans(conc.wt[, 3:136], na.rm = TRUE)
st_conc <- apply(conc.wt[, 3:136], 2, sd, na.rm = TRUE)
...
# Save plot in folder
ggsave("Output/Plots/Teachers/WTPCBi.png", plot = plot.pcbi,
       width = 10, height = 5, dpi = 1200)

# Concentration Profile Analysis ------------------------------------------
# Profiles
tmp <- rowSums(conc.wt[, 3:136], na.rm = TRUE)
prof.wt.conc <- sweep(conc.wt[, 3:136], 1, tmp, FUN = "/")
prof.wt.conc <- cbind(conc.wt$code.teacher, prof.wt.conc)
# Check sum of all PCBs (i.e., = 1)
rowSums(prof.wt.conc[, 2:135], na.rm = TRUE)
...
# PCA
t.prof.wt.conc <- data.frame(t(prof.wt.conc[, 2:135]))
...
# Export
write.csv(costheta_df, file = "Output/Data/csv/Teachers/CosineThetaTeachers.csv")
...
# Plot the data
plot.cos.theta.low <- ggplot(data = costheta_correlations[1:16, ],
...
# Save plot
ggsave("Output/Plots/Profiles/Teachers/CosThetaLowV01.png", plot = plot.cos.theta.low,
       width = 10, height = 10, dpi = 1200)

plot.cos.theta.high <- ggplot(data = costheta_correlations[613:630, ],
                              aes(x = Var1, y = Var2, fill = Value)) +
  geom_tile(color = "white") +
  scale_fill_gradient(low = "blue", high = "red") +
  labs(x = "", y = "") +
...
# Save plot
ggsave("Output/Plots/Profiles/Teachers/CosThetaHighV01.png", plot = plot.cos.theta.high,
       width = 10, height = 10, dpi = 1200)

# Plot Individual PCB Profiles --------------------------------------------
# Select rows for the sample
selected_rows <- prof.wt.conc %>%
  filter(conc.wt$code.teacher %in% c("wt.25.r")) # need to change the sample!
...
# Save plot
ggsave("Output/Plots/Profiles/Teachers/wt25r.png", plot = plot.25.r,
