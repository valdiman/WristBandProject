...
ko.p <- ko.p[c(1,7)]
{
  ...
  veff.V1.o <- 10^(logKwb$logKwb) * Vwb.V1 * 
    (1 - exp(-ko.p$Average_ko2 * Awb.V1 / Vwb.V1 / 10^(logKwb$logKwb) * data.2$time[4] / 24))
  veff.V1.h <- 10^(logKwb$logKwb) * Vwb.V1 * 
    (1 - exp(-ko.p$Average_ko2 * Awb.V1 / Vwb.V1 / 10^(logKwb$logKwb) * data.2$time[5] / 24))
  Vwb.V2 <- data.2$vol.WB[6] # [m3]
  Awb.V2 <- data.2$area.WB[6] # [m2]
  veff.V2.o <- 10^(logKwb$logKwb) * Vwb.V2 * 
    (1 - exp(-ko.p$Average_ko2 * Awb.V2 / Vwb.V2 / 10^(logKwb$logKwb) * data.2$time[7] / 24))
  veff.V2.h <- 10^(logKwb$logKwb) * Vwb.V2 * 
    (1 - exp(-ko.p$Average_ko2 * Awb.V2 / Vwb.V2 / 10^(logKwb$logKwb) * data.2$time[6] / 24))
  Vwb.V3 <- data.2$vol.WB[8] # [m3]
  Awb.V3 <- data.2$area.WB[8] # [m2]
  veff.V3.o <- 10^(logKwb$logKwb) * Vwb.V3 * 
    (1 - exp(-ko.p$Average_ko2 * Awb.V3 / Vwb.V3 / 10^(logKwb$logKwb) * data.2$time[8] / 24))
  veff.V3.h <- 10^(logKwb$logKwb) * Vwb.V3 * 
    (1 - exp(-ko.p$Average_ko2 * Awb.V3 / Vwb.V3 / 10^(logKwb$logKwb) * data.2$time[9] /24))
  Vwb.V4 <- data.2$vol.WB[10] # [m3]
  Awb.V4 <- data.2$area.WB[10] # [m2]
  veff.V4.o <- 10^(logKwb$logKwb) * Vwb.V4 * 
    (1 - exp(-ko.p$Average_ko2 * Awb.V4 / Vwb.V4 / 10^(logKwb$logKwb) * data.2$time[11] / 24))
  veff.V4.h <- 10^(logKwb$logKwb) * Vwb.V4 * 
    (1 - exp(-ko.p$Average_ko2 * Awb.V4 / Vwb.V4 / 10^(logKwb$logKwb) * data.2$time[10] / 24))
  Vwb.V5 <- data.2$vol.WB[12] # [m3]
  Awb.V5 <- data.2$area.WB[12] # [m2]
  veff.V5.o <- 10^(logKwb$logKwb) * Vwb.V5 * 
    (1 - exp(-ko.p$Average_ko2 * Awb.V5 / Vwb.V5 / 10^(logKwb$logKwb) * data.2$time[13] / 24))
  veff.V5.h <- 10^(logKwb$logKwb) * Vwb.V5 * 
    (1 - exp(-ko.p$Average_ko2 * Awb.V5 / Vwb.V5 / 10^(logKwb$logKwb) * data.2$time[12] / 24))
}
...
p.AirWBPCBi.volun2.Fred <- ggplot(Volunteer2_PCBi_fred, aes(x = Conc.Air, y = Conc.WB, 
  ...
  theme(aspect.ratio = 1) +
  annotation_logticks(sides = "bl") +
  scale_y_log10(limits = c(0.0001, 10^3),
                breaks = scales::trans_breaks("log10", function(x) 10^x),
                labels = scales::trans_format("log10", math_format(10^.x))) +
  scale_x_log10(limits = c(0.0001, 10^3),
                breaks = scales::trans_breaks("log10", function(x) 10^x),
                labels = scales::trans_format("log10", math_format(10^.x))) +
  xlab(expression(bold("Office Air Concentration PCBi (ng/m"^3*")"))) +
